...
- **Don't overuse the em-dash.** It's the biggest "AI wrote this" tell. Em-dashes are fine in headings, subtitles, and short CTA labels, but in body copy (feature `text`, `before`/`after`, FAQ answers, `cost_line`, `lead`, `final.text`) default to periods, commas, or colons instead — aim for at most one or two em-dashes across all body fields on the whole page. Never alter the punctuation of real review quotes, and keep en-dashes in numeric ranges. See [copywriting.md](./references/copywriting.md#punctuation-go-easy-on-the-em-dash).
