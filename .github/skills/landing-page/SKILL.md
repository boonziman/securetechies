...
- **Images that need cropping must live in `assets/`, not `static/`.** `resources.Get` returns `nil` for files under `static/`, so Hugo can't `Fill`/resize them — they get served raw and look blurry when the template stretches them. Put any photo you intend to crop in `assets/images/...` so `Fill "WxH webp qNN"` produces a sharp, correctly-sized webp. The benefits image renders as a **tall, full-height frame that bleeds to the left screen edge** (`.st-l-split-media` uses `align-self: stretch` + a viewport-based negative `margin-left`), so `Fill` it at a tall size (e.g. `760x620`+) and pick a photo that crops well when tall (people/office/team). **Never upscale** beyond the source resolution, or it blurs.
